...
//	var $cache = 'cache/'; // Cache folder
  ...
  * Date: June 02, 2010
  * GETTER SETTER METHOD, SOME VALUE HAS ALREADY GETTER SETTER PLEASE LOOK FURTHER
  **/
  function setDebug($v)
  {
    $this->debug = $v;
  ...
  function copyImage($image, $thumb = '') {
		if (!($this->data = $this->itsImage($image))) {
			return $this->debug($image.' isn\'t a valid image');
		}
	...
	*/
    ...
    function upload($upfile,$path,$new_name,$key="NULL",$type="NULL") //$upfile = $_FILES['input_file_name']['name'],$path = folder where main image is going to be uploadaed, $new_name = new name for the file to make it unique *Without extension
    {
/////////////////////////////for multiple file send $key and $type 
        $fo=$path;
        $f=$type=="NULL"?$_FILES[$upfile]['name']:$_FILES[$upfile]['name'][$key];
        $arr=array();
        $arr=explode(".",$f);
        if(!in_array(end($arr),$this->allowed))
        {
            return $this->debug($upfile.' isn\'t a valid image');
        }
        $file=$new_name.'.'.end($arr);
        $uf=$fo.$file;
        $tmp_file=$type=="NULL"?$_FILES[$upfile]['tmp_name']:$_FILES[$upfile]['tmp_name'][$key];
        $this->main_img = $file;
        $this->main_src = $uf;
        return move_uploaded_file($tmp_file,$uf);
