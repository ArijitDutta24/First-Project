...
$GATEWAY->lang         = array(trim(str_replace('{store}', '', $public_checkout18)));
...
if (!isset($_POST['custom'])) {
  ...
  if ($input) {
    ...
    if (isset($hookevent['object'], $hookevent['type']) && $hookevent['object'] == 'event') {
      define('HOOK_EVENT', $hookevent['type']);
...
if ($INCOMING['code-id']) {
  ...
  // DEBUG..
  $GATEWAY->writeLog($SALE_ID, 'Received ' . (defined('HOOK_EVENT') ? 'webhook event' : 'post') . ' callback from ' . $GATEWAY->gateway_name . ' payment server..writing post log..');
...
// Stripe expects a JSON response for payment..
// For webhook standard 200..
if (!defined('HOOK_EVENT')) {
  // Clear basket..
  $MCCART->clearCart();
  if (isset($SALE_ID) && $SALE_ID > 0) {
    if (!class_exists('jsonHandler')) {
      include(PATH . 'control/classes/class.json.php');
      $MCJSON = new jsonHandler();
    }
    echo $MCJSON->encode(array(
      'status' => (isset($paymentStatus) && $paymentStatus == 'completed' ? 'ok' : 'err'),
      'title' => $mc_global[2],
      'msg' => $mc_34_admin[3],
      'url' => $SETTINGS->ifolder . '/index.php?gw=' . $SALE_ID . '-' . $SALE_CODE,
      'errcode' => (isset($paymentStatus) && $paymentStatus == 'declined_by_gateway' ? $CALLBACK['fail-code'] : '')
    ));
  } else {
